...
@step(u'And the previous bindings are published for the context (\d+):')
def and_the_previous_bindings_are_published_for_the_context_operation_index(step, context_index):
    api_utils.send_to_url_the_rule_data(step, "$base_api_url/$bindings_url", context_index)


@step(u'the result set contains the binding (\d+):')
def the_resultset_contains_binding(step, binding_index):
    api_utils.the_resultset_contains_binding_in_position(step, binding_index)
