...
def a_class_has_already_been_published_with_data(step, old_class_index):
    """ Rest world variables for scenario execution"""
...
@step(u'I try to partially update in (.*) the binding data (\d+):')
def i_try_to_partially_update_the_binding_data(step, url, binding_index):
    api_utils.send_to_url_the_rule_data(step, url, binding_index)
...
@step(u'I put in (.*) the binding data (\d+):')
def i_put_in_url_the_binding_data(step, url, binding_index):
    api_utils.put_in_url_the_binding_data(step, url, binding_index)


@step(u'I put in (.*) the data (.*)')
def i_put_in_url_the_data(step, url, data):
    rest_utils.put_in_url_the_data(step, url, data)


@step(u'the URL (.*) returns the updated binding data')
def the_url_returns_the_updated_binding_data(step, url):
    api_utils.the_url_returns_the_rule_data(step, url)


@step(u'I get a success response of type (\d+) with the updated binding data')
def i_get_a_success_response_of_type_with_updated_binding_data(step, status_code):
    api_utils.i_get_a_success_response_of_type_with_updated_binding_data(step, status_code)
