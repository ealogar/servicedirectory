...
@step(u'I get the service class collection (.*)')
def i_get_the_service_class_collection(step, url):
    api_utils.the_url_returns_the_class_collection(step, url)


@step(u'I request the resource (.*)')
def i_request_the_resource(step, url):
    rest_utils.request_the_resource(step, url)


@step(u'I request the resource (.*) with headers (\d+):')
def i_request_the_resource_with_extra_header(step, url, headers_index):
    rest_utils.request_the_resource(step, url, headers_index=headers_index)


@step(u'the collection contains the class (.*)')
def the_collection_contains_the_class(step, class_name):
    api_utils.the_class_collection_contains_the_class(step, class_name)


@step(u'the collection not contains the class (.*)')
def the_collection_not_contains_the_class(step, class_name):
    api_utils.the_class_collection_not_contains_the_class(step, class_name)
...
@step(u'I get an error response of type (\d+)')
def i_get_an_error_response_of_type(step, status_code):
...
@step(u'another class has already been published with data (\d+):')
