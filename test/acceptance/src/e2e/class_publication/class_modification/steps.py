...
def a_class_has_already_been_published_with_data(step, old_class_index):
    ...
    world.base_class = copy.copy(world.json)


@step(u'I try to put in (.*) the class data (\d+):')
def i_put_to_url_the_class_data(step, url, class_index):
...
@step(u'the URL (.*) returns the updated class data')
def the_url_returns_the_updated_class_data(step, url):
    api_utils.the_url_returns_the_class_data(step, url)


@step(u'I get a success response of type (\d+) with the updated class data')
def i_get_a_success_response_of_type_with_updated_class_data(step, status_code):
    api_utils.i_get_a_success_response_of_type_with_updated_class_data(step, status_code)
...
@step(u'the location returns the class data')
def the_location_returns_the_class_data(step):
    api_utils.the_url_returns_the_class_data(step, world.location)
