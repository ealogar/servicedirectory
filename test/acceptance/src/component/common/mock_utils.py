...
class SDMockUtils(object):

    def start_sd_mock(self):
        """
        Start the SD mock if not already started and store the process in the world.
        """
        if not hasattr(world, "sd_mock_working") or world.sd_mock_working == False:
            run_command = " ".join([world.config["environment"]["python_alias"],
                                   world.config["sd_mock"]["run_command_params"]])
            out = open(world.config["sd_mock"]["out_file"], 'w')
            err = open(world.config["sd_mock"]["err_file"], 'w')
            world.sd_mock_process = subprocess.Popen(run_command.split(), stdout=out, stderr=err)
            time.sleep(1)
            world.sd_mock_working = True

    def stop_sd_mock(self):
        """
        Stop the SD mock if not already stopped.
        """
        if hasattr(world, "sd_mock_working") and world.sd_mock_working == True:
            world.sd_mock_process.terminate()
            time.sleep(0.5)
            world.sd_mock_working = False

    def sd_mock_is_working(self):
        """
        Check whether the SD mock is working
        """
        if hasattr(world, "sd_mock_working") and world.sd_mock_working == True:
            return True
        else:
    ...
    def set_invalid_data(self, type,  request, status_code, delay=None):
        """
        Configure the SD mock to give a bad answer to a request.
        ...
        """Send the configuration request to the SD mock."""
        url = "".join([world.config["sd_mock"]["base_url"], "/mock_configurations/", request])
        try:
            requests.post(url, json.dumps(response_info))
        except:
            assert False, "Error configuring the SD mock. URL: %s. Data: %s" \
                % (url, response_info)
    ...
    def get_request_and_send_response_of_type_with_data(self, step, request,
                                                        ...
                                                        delay=None):
        """
        Configure the SD mock to give a specific answer to a request.
    ...
    def _get_url_with_values(self, url_template):
        ...
        return temp.safe_substitute(apis_url=world.config["django"]["apis_url"],
    ...
    def validate_stored_request(self, request, params=None, body_data=None):
        ...
        try:
            ...
            assert False, "Error getting stored info from SD mock."
        ...
        assert response.status_code == 200, "Error getting stored info from SD mock."
