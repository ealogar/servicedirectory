...
@step(u'the SD is ready to return the class (.*):')
def the_SD_is_i_ready_to_return_a_specific_class(step, class_name):
    mock_utils.get_request_and_send_response_of_type_with_data(step, "sd/v1/classes/" + class_name, 200, 0)


@step(u'the SD is ready to return the updated class (.*):')
def the_SD_is_i_ready_to_return_the_updated_class(step, class_name):
    mock_utils.get_request_and_send_response_of_type_with_data(step, "sd/v1/classes/" + class_name, 200, 0)
...
@step(u'the SD is ready to return the created instance:')
...
@step(u'the SD is ready to return the binding:')
def the_SD_is_i_ready_to_return_the_binding(step):
    mock_utils.get_request_and_send_response_of_type_with_data(step, "sd/v1/bindings", 201, 0)


@step(u'the SD is ready to return no bindings')
def the_SD_is_i_ready_to_return_no_bindings(step):
    mock_utils.get_request_and_send_response_of_type_with_data(step, "sd/v1/bindings", 200)


@step(u'the SD is ready to return the following bindings:')
def the_SD_is_i_ready_to_return_the_following_bindings(step):
    mock_utils.get_request_and_send_response_of_type_with_data(step, "sd/v1/bindings", 200)


@step(u'the SD is ready to return the binding when is searched:')
def the_SD_is_i_ready_to_return_the_binding_when_is_searched (step):
    mock_utils.get_request_and_send_response_of_type_with_data(step, "sd/v1/bindings", 200)


@step(u'the SD is ready to return the updated binding (.*):')
def the_SD_is_i_ready_to_return_the_updated_binding(step, binding_id):
    mock_utils.get_request_and_send_response_of_type_with_data(step, "sd/v1/bindings/" + binding_id, 200, 0)    
    
@step(u'the SD is ready to return an error (\d+) for binding discovery:')
def the_SD_is_i_ready_to_return_an_error_for_binding_discovery(step, error_code):
    mock_utils.get_request_and_send_response_of_type_with_data(step, "sd/v1/bindings", error_code, 0)


@step(u'the SD is ready to return an error (\d+) for binding deletion of binding (.*):')
def the_SD_is_i_ready_to_return_an_error_for_binding_deletion(step, error_code, binding_id):
    mock_utils.get_request_and_send_response_of_type_with_data(step, "sd/v1/bindings/" + binding_id, error_code, 0)
